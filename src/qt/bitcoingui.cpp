 ...
 * The Bungee Developers 2018-2019
 */
...
BungeeGUI::BungeeGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(900, 520);
    setWindowTitle(tr("Bungee") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Bungee");
    setStyleSheet("#Bungee { background-color: #ffffff; color: #0e0695;}");
    ...
    if (!fUseDarkTheme)
    {
        ...
        {
            progressBar->setStyleSheet("QProgressBar { color: #0e0695;background-color: #e8e8e8; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #eb1f24, stop: 1 #4c5259); border-radius: 7px; margin: 0px; }");
        }
    }
    ...
    statusBar()->setStyleSheet("#statusBar { color: #2a21bc; background-color: #1d1f22; }");

    if (!fUseDarkTheme)
    {
        statusBar()->setStyleSheet("#statusBar { color: #0e0695; background-color: #ffffff; }");
    }
...
void BungeeGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Bungee address"));
    ...
    TradingAction = new QAction(QIcon(":/icons/trade"), tr("&Exchange"), this);
    ...
    aboutAction = new QAction(QIcon(fUseDarkTheme ? ":/icons/dark/bitcoin-dark" : ":/icons/bitcoin"), tr("&About Bungee"), this);
    aboutAction->setToolTip(tr("Show information about Bungee"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Bungee"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Bungee.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for Bungee"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Bungee.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for Bungee (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where Bungee data is stored"));
...
void BungeeGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #ffffff; font-weight:bold; background-color: #121418;} QToolButton:hover { background-color: #362dce; } QToolButton:checked { background-color: #362dce } QToolButton:pressed { background-color: #362dce; } #tabs { color: #ffffff; background-color: #121418; }");
    toolbar->setIconSize(QSize(24,24));

    if(!fUseDarkTheme)
    {
        toolbar->setStyleSheet("QToolButton { color: #0e0695; font-weight:bold; } QToolButton:hover { background-color: #362dce; } QToolButton:checked { background-color: #1e13e1; } QToolButton:pressed { background-color: #140bb2; } #tabs { color: #0e0695; background-color: #ffffff; }");
    }
    ...
    netLabel->setStyleSheet("#netLabel { color: #0e0695; }");
...
void BungeeGUI::setClientModel(ClientModel *clientModel)
{
    if(!fOnlyTor)
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Bungee client") + QString(" ") + tr("[testnet]"));
...
void BungeeGUI::setWalletModel(WalletModel *walletModel)
{
...
void BungeeGUI::setMessageModel(MessageModel *messageModel)
{
...
void BungeeGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Bungee client"));
...
#ifndef Q_OS_MAC
void BungeeGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BungeeGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Bungee network", "", count));
}

void BungeeGUI::setNumBlocks(int count)
{
    QString tooltip;
    ...
    else
    {
        ...
        progressBarLabel->setStyleSheet("QLabel { color: #0e0695; }");
...
void BungeeGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Bungee") + " - ";
...
void BungeeGUI::error(const QString &title, const QString &message, bool modal)
{
...
void BungeeGUI::changeEvent(QEvent *e)
{
...
void BungeeGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BungeeGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(BungeeUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void BungeeGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(BungeeUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void BungeeGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BungeeGUI::gotoMasternodeManagerPage()
{
...
void BungeeGUI::gotoBlockBrowser()
{
...
void BungeeGUI::gotoOverviewPage()
{
...
void BungeeGUI::gotoHistoryPage()
{
...
void BungeeGUI::gotoAddressBookPage()
{
...
void BungeeGUI::gotoTradingPage()
{
...
void BungeeGUI::gotoReceiveCoinsPage()
{
...
void BungeeGUI::gotoSendCoinsPage()
{
...
void BungeeGUI::gotoSignMessageTab(QString addr)
{
...
void BungeeGUI::gotoVerifyMessageTab(QString addr)
{
...
void BungeeGUI::gotoMessagePage()
{/*
...
void BungeeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BungeeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Bungee address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BungeeGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void BungeeGUI::setEncryptionStatus(int status)
{
...
void BungeeGUI::importPrivateKey()
{
...
void BungeeGUI::changePassphrase()
{
...
void BungeeGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BungeeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BungeeGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BungeeGUI::updateStakingIcon()
{
    updateWeight();
...
void BungeeGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void BungeeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
