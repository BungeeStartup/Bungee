...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBungeeAmount"))
        settings.setValue("nAnonymizeBungeeAmount", 1000);
    nAnonymizeBungeeAmount = settings.value("nAnonymizeBungeeAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBungeeAmount"))
        SoftSetArg("-anonymizeBungeeamount", settings.value("nAnonymizeBungeeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBungeeAmount:
            return QVariant(nAnonymizeBungeeAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeBungeeAmount:
            nAnonymizeBungeeAmount = value.toInt();
            settings.setValue("nAnonymizeBungeeAmount", nAnonymizeBungeeAmount);
            emit AnonymizeBungeeAmountChanged(nAnonymizeBungeeAmount);
            break;
        default:
            break;
        }
