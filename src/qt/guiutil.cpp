...
{
    widget->setMaxLength(BungeeAddressValidator::MaxAddressLength);
    widget->setValidator(new BungeeAddressValidator(parent));
...
bool parseBungeeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("Bungee"))
        return false;
...
bool parseBungeeURI(QString uri, SendCoinsRecipient *out)
{
    // Convert bungee:// to Bungee:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bungee://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Bungee:");
    }
    QUrl uriInstance(uri);
    return parseBungeeURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bungee.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Bungee.lnk
...
{
    return GetAutostartDir() / "Bungee.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Bungee.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Bungee\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Bungee app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Bungee app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bungee-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bungee-qt [" + tr("command-line options") + "]                     " + "\n";
