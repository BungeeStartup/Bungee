...
namespace {
    class CBungeeAddressVisitor : public boost::static_visitor<bool> {
    private:
        CBungeeAddress *addr;
    public:
        CBungeeAddressVisitor(CBungeeAddress *addrIn) : addr(addrIn) { }
...
bool CBungeeAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CBungeeAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CBungeeAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CBungeeAddressVisitor(this), dest);
}

bool CBungeeAddress::IsValid() const {
...
CTxDestination CBungeeAddress::Get() const {
    if (!IsValid())
...
bool CBungeeAddress::GetKeyID(CKeyID &keyID) const {
...
bool CBungeeAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CBungeeSecret::SetKey(const CKey& vchSecret) {
...
CKey CBungeeSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CBungeeSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CBungeeSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBungeeSecret::SetString(const std::string& strSecret) {
